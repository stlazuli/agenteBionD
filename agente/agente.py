...
# Caminho para a pasta de skills
...
# Instância do banco de dados SQLite (compartilhada)
db = SqliteDb(
    db_file=str(DB_PATH),
...
def run_interactive():
    console.print(
        Panel(
            "[bold green]BionD Agent[/bold green]\n"
            "Assistente de qualificação de leads\n\n"
            "[dim]Digite 'sair' ou 'exit' para encerrar[/dim]",
            title="Bem-vindo",
            border_style="green",
        )
    )
    ...
    while True:
        try:
            ...
            console.print()
            console.print("[bold green]BionD:[/bold green]")

            if response and response.content:
                console.print(response.content)
            else:
                console.print("[red]Sem resposta do agente[/red]")
